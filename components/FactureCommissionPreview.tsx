...
import { Facture, FactureType, FACTURE_TYPE_OPTIONS, COMMISSION_RATES } from "@/types/facture";
...
import { Building, Download, Edit, User as UserIcon, FileText, Percent } from "lucide-react";
import { Timestamp } from "firebase/firestore";
import { getOrCreateFactureCommissionData, FactureCommissionData } from "@/utils/factureCommissionPersistence";
...
// Informations par défaut de l'entreprise pour Aximotravo
export const AXIMOTRAVO_COMPANY_INFO = {
...
export const FactureCommissionPreview: React.FC<FactureCommissionPreviewProps> = ({
  userId,
  devis,
  factureType,
  onClose,
  ...
  const [factureData, setFactureData] = useState<FactureCommissionData | null>(null);
  const [tauxCommission, setTauxCommission] = useState<number>(
    factureType === 'commission_courtier' ? COMMISSION_RATES.commission_courtier : COMMISSION_RATES.commission_aximotravo
  );
  ...
  const getClientCode = () => {
    if (factureType === 'commission_courtier' && user) {
      ...
      return (firstName.substring(0, 2) + lastName.substring(0, 2)).toUpperCase() || 'COUR';
    }
    return 'AXIM';
  };
        ...
        <div className="p-4 sm:p-6 border-b">
          <div className="flex flex-col sm:flex-row justify-between items-center sm:items-start gap-4">
            ...
            <div className="flex-shrink-0">
              <img
                src={AXIMOTRAVO_COMPANY_INFO.logoUrl}
                alt="AXIMOTRAVO"
            ...
            <div className="text-center sm:text-right">
              <div className="border border-orange-200 p-3 sm:p-4 bg-orange-50">
                <h1 className="text-base sm:text-lg font-bold mb-1 text-[#F26755]">Facture</h1>
                <p className="text-base sm:text-lg font-bold text-[#F26755]">{generateFactureNumber()}</p>
                <p className="text-xs sm:text-sm mt-2 text-orange-700">{formatDate(new Date())}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6">
            ...
            <div>
              <div className="border border-orange-200">
                <div className="bg-orange-100 px-3 py-1 border-b border-orange-200">
                  <h3 className="font-bold text-xs sm:text-sm text-[#F26755]">ÉMETTEUR</h3>
                </div>
                <div className="p-3 text-xs">
                  <p className="font-bold">{AXIMOTRAVO_COMPANY_INFO.name}</p>
                  <p>{AXIMOTRAVO_COMPANY_INFO.address}</p>
                  <p>{AXIMOTRAVO_COMPANY_INFO.postalCode} {AXIMOTRAVO_COMPANY_INFO.city}</p>
                  <p>SIRET: {AXIMOTRAVO_COMPANY_INFO.siret}</p>
                  <p>TVA: FR12345678901</p>
                  <p>Tél: {AXIMOTRAVO_COMPANY_INFO.phone}</p>
                  <p>Email: {AXIMOTRAVO_COMPANY_INFO.email}</p>
                </div>
              </div>
            </div>
            ...
            <div>
              <div className="border border-orange-200">
                <div className="bg-orange-100 px-3 py-1 border-b border-orange-200">
                  <h3 className="font-bold text-xs sm:text-sm text-[#F26755]">ADRESSE DE FACTURATION</h3>
                </div>
                <div className="p-3 text-xs">

                    <>
                      <p className="font-bold">{artisan?.companyName || `${artisan?.firstName} ${artisan?.lastName}`}</p>
                      {artisan?.companyAddress && <p>{artisan?.companyAddress}</p>}
                      {artisan?.companyPostalCode && artisan?.companyCity && (
                        <p>{artisan?.companyPostalCode} {artisan?.companyCity}</p>
                      )}
                      {artisan?.siret && <p>SIRET: {artisan?.siret}</p>}
                      {artisan?.companyPhone && <p>Tél: {artisan?.companyPhone}</p>}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-orange-100 border-b border-orange-200">
                      <th className="border-r border-orange-200 px-3 py-2 text-left font-medium text-[#F26755]">Désignation</th>
                      <th className="border-r border-orange-200 px-3 py-2 text-center font-medium text-[#F26755]">Prix unitaire € HT</th>
                      <th className="border-r border-orange-200 px-3 py-2 text-center font-medium text-[#F26755]">Qté</th>
                      <th className="border-r border-orange-200 px-3 py-2 text-center font-medium text-[#F26755]">Prix total € HT</th>
                      <th className="px-3 py-2 text-center font-medium text-[#F26755]">TVA</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      <td className="border-r border-orange-200 px-3 py-4 align-top">
                        ...
                        <p className="text-xs mt-1">
                          Commissionnement de {tauxCommission}% sur le montant HT des travaux: {formatPrice(devisTotals.totalHT)} suivant
                        </p>
                        <p className="text-xs">
                          le devis n° {devis.numero} daté du {formatDate(devis.createdAt)} et accepté le
                        </p>
                        <p className="text-xs">
                      ...
                      <td className="border-r border-orange-200 px-3 py-4 text-center align-top">
                        {formatPrice(montantCommissionHT).replace('€', '').trim()}
                      </td>
                      <td className="border-r border-orange-200 px-3 py-4 text-center align-top">
                        1
                      </td>
                ...
                <div className="p-4 space-y-4">
                  <div className="bg-white border border-orange-200 rounded p-3">
                    ...
                    <div className="text-xs space-y-1 mb-3">
                      <p>Client: {client?.firstName?.toUpperCase()} {client?.lastName?.toUpperCase()}</p>
                      <p>Commissionnement de {tauxCommission}% sur le montant HT des travaux: {formatPrice(devisTotals.totalHT)} suivant</p>
                      <p>le devis n° {devis.numero} daté du {formatDate(devis.createdAt)} et accepté le {formatDate(devis.updatedAt || devis.createdAt)}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="bg-orange-50 p-2 rounded">
                        <span className="text-[#F26755] font-medium">Prix unitaire HT:</span>
                        <br />{formatPrice(montantCommissionHT).replace('€', '').trim()}
                      </div>
                      <div className="bg-orange-50 p-2 rounded">
                        <span className="text-[#F26755] font-medium">Quantité:</span>
                        <br />1
                      </div>
                      <div className="bg-orange-50 p-2 rounded">
                        <span className="text-[#F26755] font-medium">Total HT:</span>
                        <br />{formatPrice(montantCommissionHT).replace('€', '').trim()}
                      </div>
                      <div className="bg-orange-50 p-2 rounded">
          ...
          <div className="mb-6">
            <div className="border border-orange-200">
              <div className="bg-orange-100 px-3 py-2 border-b border-orange-200">
                <h3 className="font-bold text-sm md:text-base text-[#F26755]">Conditions de paiement</h3>
              </div>
              <div className="p-3 md:p-4 text-xs md:text-sm space-y-2">
                <div className="bg-orange-50 p-3 rounded border-l-4 border-[#F26755]">
                  <p className="font-medium text-[#F26755] mb-2">Modalités de règlement :</p>
                  <p className="leading-relaxed">
                    Selon articles L 441-10 et suivants du code du commerce, taux appliqué : <span className="font-medium">15,21% par an</span>.
                  </p>
                  <p className="leading-relaxed mt-2">
                    Une indemnité forfaitaire de <span className="font-medium text-[#F26755]">40 €</span> sera due de plein droit dès le premier jour de retard de paiement 
                    (Article D. 441-5 du code du commerce).
                  </p>
                </div>
                <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-2 pt-2 border-t border-orange-200">
                  <span className="font-medium text-[#F26755]">Mode de règlement :</span>
                  <span>Virement bancaire ou chèque</span>
                </div>
                <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-2">
                  <span className="font-medium text-[#F26755]">Délai de paiement :</span>
          ...
          <div className="flex justify-center md:justify-end">
            ...
            <div className="w-full md:w-auto md:min-w-[400px]">
              <div className="border border-orange-200">
                <div className="bg-orange-100 px-3 py-2 border-b border-orange-200">
                  <h3 className="font-bold text-sm md:text-base text-[#F26755]">Montant total lignes HT</h3>
                  <div className="text-right font-bold text-lg text-[#F26755]">{formatPrice(montantCommissionHT)}</div>
                </div>
                <div className="p-3 text-xs md:text-sm space-y-2">
                  ...
                  <div className="hidden md:block">
                    ...
                    <div className="grid grid-cols-5 gap-1 mt-1">
                      ...
                      <div className="text-center">{formatPrice(montantCommissionTTC).replace('€', '').trim()}</div>
                      <div className="text-center">{formatPrice(montantCommissionHT).replace('€', '').trim()}</div>
                      <div className="text-center">{formatPrice(montantCommissionHT * 0.20).replace('€', '').trim()}</div>
                    </div>
                  </div>
                  ...
                  <div className="md:hidden mt-2 border-t pt-2">
                    <div className="bg-orange-50 p-2 rounded mb-2">
                      <div className="font-medium text-[#F26755] mb-1">Détail TVA</div>
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div>Code: A (20%)</div>
                        <div>TVA: {formatPrice(montantCommissionHT * 0.20)}</div>
                  ...
                  <div className="border-t pt-2 mt-2">
                    <div className="flex justify-between font-bold text-sm md:text-base">
                      ...
                      <span className="text-[#F26755]">{formatPrice(montantCommissionTTC)}</span>
                    </div>
                  </div>
                  <div className="mt-2 text-xs md:text-sm">
                    <p>Règlement: à réception de facture</p>
                    <p className="mt-1 font-medium text-[#F26755]">Facture à régler</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="mt-6 border-t pt-4">
            <div className="text-center text-xs md:text-sm text-gray-600 space-y-1">
              <div className="flex flex-col md:flex-row md:justify-center md:items-center md:gap-2">
                <span className="font-medium text-[#F26755]">{AXIMOTRAVO_COMPANY_INFO.name.toUpperCase()}</span>
                <span className="hidden md:inline">•</span>
                <span>{AXIMOTRAVO_COMPANY_INFO.companyLegalForm} au capital de {AXIMOTRAVO_COMPANY_INFO.companyCapital}€</span>
              </div>
              <div className="flex flex-col md:flex-row md:justify-center md:items-center md:gap-2">
                <span>www.aximotravo.com</span>
                <span className="hidden md:inline">•</span>
                <span>contact@aximotravo.com</span>
              </div>
              <div className="flex flex-col md:flex-row md:justify-center md:items-center md:gap-2">
                <span>{AXIMOTRAVO_COMPANY_INFO.address}</span>
                <span className="hidden md:inline">•</span>
                <span>{AXIMOTRAVO_COMPANY_INFO.postalCode} {AXIMOTRAVO_COMPANY_INFO.city} France</span>
                <span className="hidden md:inline">•</span>
                <span>tél {AXIMOTRAVO_COMPANY_INFO.phone}</span>
              </div>
              <div className="flex flex-col md:flex-row md:justify-center md:items-center md:gap-2 pt-1 border-t border-gray-200">
                <span>RCS {AXIMOTRAVO_COMPANY_INFO.rcs}</span>
                <span className="hidden md:inline">•</span>
                <span>SIREN {AXIMOTRAVO_COMPANY_INFO.siret?.substring(0, 9)}</span>
                <span className="hidden md:inline">•</span>
                <span>APE {AXIMOTRAVO_COMPANY_INFO.companyApe}</span>
              </div>
            </div>
          </div>
        </div>
