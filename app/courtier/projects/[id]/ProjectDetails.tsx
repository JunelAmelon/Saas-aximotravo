...
export default function ProjectDetails({ id }: { id: string }) {
  ...
  const [availableArtisans] = useState<Artisan[]>([
    { id: "1", name: "Pierre Martin", company: "Martin Construction" },
    { id: "2", name: "Jean Dupont", company: "Dupont Rénovation" },
    { id: "3", name: "Marie Laurent", company: "Laurent & Fils" },
  ]);
  ...
  const handleArtisanSelect = (artisanId: string) => {
    ...
    const selectedArtisan = availableArtisans.find(a => a.id === artisanId);
    if (selectedArtisan) {
      setProject(prev => ({
        ...prev,
        actors: {
  ...
  const handleSendRequest = async () => {
    ...
    const selectedArtisan = availableArtisans.find(a => a.id === selectedArtisanId);
